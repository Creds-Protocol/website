...
contract Airdrop {
    ...
    constructor(address _admin, uint256 _tokensPerMatic) {
        admin = _admin;
    ...
    function airdrop(address[] calldata _account, uint256[] calldata _amount) public {
        require(_account.length == _amount.length, "INVALID_INPUT");
        for(uint256 i = 0; i < _account.length; i++){
            allotToken(_account[i], _amount[i]);
        }
    }
    ...
    function getUserTokensAvailableForMinting(address _address) public view returns(uint256) {
        uint256 amount = 0;
        ...
        for (uint256 i = 0; i < userAllotmentIds[_address].length; i++) {
            ...
            uint256 oneDayAllotment = allotments[userAllotmentIds[_address][i]].tokenAlloted.div(730);
